...
DWORD WINAPI RecvFileData(LPVOID info)
{
	...
	if (recv_data >
		...
		- client_info->file_info.is_file_send) //데이터가 file_size/5 이하로 남았을 시
	...
	retval = recvn(client_info->sock, (void*)(client_info->file_info.buffer), recv_data, 0);
	count=fwrite((void*)(client_info->file_info.buffer),1, recv_data, client_info->file_info.file);
	client_info->file_info.is_file_send += recv_data;
	printf("retval: %zd, 2. count: %zd recv data: %d\n", retval, count, client_info->file_info.is_file_send);
